...
async function loadSettings() {
  settings = await api('GET', '/api/tracker/settings');
  updateAgeDosageTag();
}

async function loadMedications() {
  medications = await api('GET', '/api/tracker/medications');
  renderList();
}
...
function medCard(med) {
  const thumb = med.imageFile
    ? `<img class="med-thumb" src="/tracker-images/${med.imageFile}" alt="${escHtml(med.name)}" />`
...
async function handleToggle(id) {
  const result = await api('POST', `/api/tracker/medications/${id}/toggle`, {});
...
async function confirmInteraction() {
  ...
  await api('POST', `/api/tracker/medications/${pendingToggleId}/toggle`, { confirmed: true });
...
function openEditModal(id) {
  ...
  // Show existing image
  if (med.imageFile) {
    showImagePreview(`/tracker-images/${med.imageFile}`);
  }
...
async function saveMed() {
  ...
  try {
    if (editingId) {
      await api('PUT', `/api/tracker/medications/${editingId}`, payload);
    } else {
      await api('POST', '/api/tracker/medications', payload);
    }
    closeModal();
    await loadMedications();
  } finally {
...
async function deleteMed(id) {
  ...
  await api('DELETE', `/api/tracker/medications/${id}`);
  await loadMedications();
}
...
async function parseImage(file) {
  ...
  const result = await api('POST', '/api/tracker/parse-image', formData);
...
function openViewModal(id) {
  ...
  if (med.imageFile) {
    document.getElementById('view-image').src = `/tracker-images/${med.imageFile}`;
    imgCol.hidden = false;
  } else {
    imgCol.hidden = true;
  }
...
async function saveSettings() {
  ...
  settings = await api('PUT', '/api/tracker/settings', { userAge });
  updateAgeDosageTag();
